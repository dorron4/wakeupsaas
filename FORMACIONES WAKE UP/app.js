// Global function: open resource options modal
window.openResourceOptions = function(fileName, title) {
    const actionModal = document.getElementById('resource-action-modal');
    const titleEl = document.getElementById('resource-action-title');
    const previewBtn = document.getElementById('btn-preview-resource');
    const downloadLink = document.getElementById('btn-download-resource');

    if (!actionModal || !titleEl || !previewBtn || !downloadLink) return;
    ...
    previewBtn.onclick = function() {
        ...
        const previewContainer = document.getElementById('preview-modal-content');
        
        if (previewModal && previewTitle && previewContainer) {
            ...
            if (lowFileName.endsWith('.pdf')) {
                ...
                previewContainer.innerHTML = `<iframe src="${fileName}" style="width: 100%; height: 80vh; border: none; border-radius: 8px;"></iframe>`;
            } else if (lowFileName.endsWith('.pptm') || lowFileName.endsWith('.ppt') || lowFileName.endsWith('.pptx')) {
                // PPT Preview is not reliable in browser, show message
                previewContainer.innerHTML = `
                    <div style="padding: 40px; text-align: center; background: rgba(255,255,255,0.05); border-radius: 12px;">
                        <i class='bx bxs-file-blank' style="font-size: 5rem; color: #ef4444; margin-bottom: 20px; display: block;"></i>
                        <h4 style="margin-bottom: 10px;">Previsualización no disponible</h4>
                        <p style="color: var(--text-secondary); margin-bottom: 20px;">Los archivos de PowerPoint no se pueden previsualizar directamente en el navegador.</p>
                        <a href="${fileName}" download class="btn-primary" style="display: inline-flex; align-items: center; gap: 10px; width: auto; text-decoration: none;">
                            <i class='bx bx-download'></i> Descargar para ver
                        </a>
                    </div>`;
            } else {
                // Default to image
                previewContainer.innerHTML = `<img src="${fileName}" style="max-width: 100%; height: auto; border-radius: 8px;">`;
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const CROWDFUNDING_CONFIG = {
        ...
        linkPayPal: 'https://www.paypal.me/tu_usuario', // <-- CAMBIA ESTE LINK
    ...
    function initCrowdfunding() {
        ...
        if (btnPayPal) btnPayPal.href = CROWDFUNDING_CONFIG.linkPayPal;
        if (btnStripe) btnStripe.href = CROWDFUNDING_CONFIG.linkStripe;
    }
